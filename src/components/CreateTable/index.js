...
const CreateTable = ({ RowNo, dep, month, year, ColNo }) => {
  ...
  const changeColumns = (e, prevMonth, newMonth) => {
    var prev = new Date(year, prevMonth, 0).getDate();
    var next = new Date(year, newMonth, 0).getDate();
    var val = next - prev;
    var dump_pre = prev;
    // console.log("previous month " + prev);
    // console.log("next month " + next);
    // console.log(" dump previous month " + dump_pre);
    ...
    if (val > 0) {
      ...
      for (let i = 0; i < -val; i++) {
        deleteColumn(dump_pre + 1);
        dump_pre = dump_pre - 1;
        console.log(" dump previous month inside loop " + dump_pre);
      }
    }
  };

  const addColumn = (current_index) => {
    setModified(true);
    if (columns.length === 31) {
      return toast.dark("You can add max. 31 columns!");
    }
    setColumns((prevColumns) => [...prevColumns, ` ${current_index}`]);
          ...
          <Col md={2} className="mx-auto mt-1">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                changeColumns(e, monthN, SelectedOption);
